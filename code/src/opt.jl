...
function gamma_formulation(data, K)
    ...
    @variable(model, b[N-K+1, 1:K], Bin) # Auxiliary variable used to linearize
    ...
    @objective(model, Min, sum(sum(sum(norm(sum((1/l) * (sum(γ[i,j,k,l]*(data[i,d] - data[j,d]) for j=1:N)) for l=1:N-K+1))^2 for k=1:K) for d=1:D) for i=1:N))
